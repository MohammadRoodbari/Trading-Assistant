...
def scan_egx_sector(sector: str = "", timeframe: str = "1D", limit: int = 20) -> dict:
    ...
    """
    from core.data.egx_sectors import (
        get_all_sectors,
...
def analyze_egx_index(index: str = "EGX30", timeframe: str = "1D", limit: int = 30) -> dict:
    ...
    """
    from core.data.egx_indices import EGX_INDICES, is_egx30_stock
    from core.data.egx_sectors import get_sector
    ...
    for i in range(0, len(symbols), batch_size):
        ...
        if present_tickers:
            from core.services.screener_provider import fetch_atr_for_tickers
...
def screen_egx_stocks(
    ...
    """
    from core.data.egx_sectors import get_sector, get_currency

    if not _TA_AVAILABLE:
        return {"error": "tradingview_ta is missing; run `uv sync`."}

    if index_filter:
        from core.data.egx_indices import EGX_INDICES
...
def generate_egx_trade_plan(symbol: str, timeframe: str = "1D") -> dict:
    ...
    if ind.get("ATR") is None:
        from core.services.screener_provider import fetch_atr_for_ticker
